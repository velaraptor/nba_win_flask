...
def get_predictions(networks, w, len_output, len_input, new_season_wins):
    ...
    for i in range(0, len(new_season_wins)):
        ...
        predict_array = [new_season_wins.iloc[i].name, final_predictions, prediction_array.tolist(),
                         new_season_wins.iloc[i].values.tolist()]
...
def main():
    ...
    while True:
        ...
        nn_predictions_live_df = pd.DataFrame(nn_predictions_live, columns=['team', 'predictions', 'array', 'actual_array'])
        nn_predictions_live_df['array'] = nn_predictions_live_df['array'].apply(json.dumps)
        nn_predictions_live_df['actual_array'] = nn_predictions_live_df['actual_array'].apply(json.dumps)
        ...
        y_mean = rfqr.predict(new_season_wins.iloc[:, 0:proportion].values)
        y_high = rfqr.predict(new_season_wins.iloc[:, 0:proportion].values, 85)
        y_low = rfqr.predict(new_season_wins.iloc[:, 0:proportion].values, 15)
        ...
        full_predictions.columns = ['team', 'predictions', 'array', 'actual_array', 'user', 'win',
